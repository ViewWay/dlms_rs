# SAP Assignment 接口类完整实现规范

**Class ID: 17 | Version: 0 | 优先级: 🟢 低**
...
SAP Assignment 管理 SAP（Service Access Point）分配。
...
| 2 | sap_assignment_list | array | static | x + 0x08 | SAP 分配列表 |
...
/// SAP Assignment 接口类 (Class ID: 17, Version: 0)
/// 
/// SAP Assignment 管理 SAP（Service Access Point）分配。
#[derive(Debug, Clone)]
pub struct SapAssignment {
    ...
    /// SAP 分配列表
    pub sap_assignment_list: Vec<DlmsType>,
}

impl SapAssignment {
    /// 创建新的 SAP Assignment 实例
    pub fn new(logical_name: ObisCode) -> Self {
        Self {
            logical_name,
            sap_assignment_list: Vec::new(),
        }
    }
}
...
impl CosemObject for SapAssignment {
    ...
    fn get_attribute(&self, attr_id: u8) -> Result<DlmsType, CosemError> {
        match attr_id {
            ...
            2 => Ok(self.sap_assignment_list.clone().into()),
...
mod tests {
    ...
    #[test]
    fn test_sap__assignment_class_id() {
        assert_eq!(SapAssignment::CLASS_ID, 17);
    }

    #[test]
    fn test_sap__assignment_version() {
        assert_eq!(SapAssignment::VERSION, 0);
    }

    #[test]
    fn test_sap__assignment_new() {
        let obis = ObisCode::from_str("0.0.17.0.0.255").unwrap();
        let obj = SapAssignment::new(obis.clone());
        assert_eq!(obj.logical_name, obis);
    }

    #[test]
    fn test_sap__assignment_get_logical_name() {
        let obis = ObisCode::from_str("0.0.17.0.0.255").unwrap();
        let obj = SapAssignment::new(obis.clone());
    ...
    #[test]
    fn test_sap__assignment_get_sap_assignment_list() {
        let obj = SapAssignment::new(ObisCode::from_str("0.0.17.0.0.255").unwrap());
