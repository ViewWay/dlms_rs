...
| 2 | entries | array | static | x + 0x08 | 日程条目列表 |
...
/// Schedule 接口类 (Class ID: 10, Version: 0)
/// 
/// Schedule 定义周期性执行的日程安排。
#[derive(Debug, Clone)]
pub struct Schedule {
...
mod tests {
    ...
    #[test]
    fn test_schedule_version() {
        assert_eq!(Schedule::VERSION, 0);
    }

    #[test]
    fn test_schedule_new() {
        let obis = ObisCode::from_str("0.0.10.0.0.255").unwrap();
        let obj = Schedule::new(obis.clone());
        assert_eq!(obj.logical_name, obis);
    }

    #[test]
    fn test_schedule_get_logical_name() {
        let obis = ObisCode::from_str("0.0.10.0.0.255").unwrap();
        let obj = Schedule::new(obis.clone());
    ...
    #[test]
    fn test_schedule_get_entries() {
        let obj = Schedule::new(ObisCode::from_str("0.0.10.0.0.255").unwrap());
