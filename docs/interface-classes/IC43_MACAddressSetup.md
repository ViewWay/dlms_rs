# MAC Address Setup 接口类完整实现规范

**Class ID: 43 | Version: 0 | 优先级: 🟢 低**
...
| 2 | mac_address | octet-string | static | x + 0x08 | MAC 地址 (6字节) |
...
/// MAC Address Setup 接口类 (Class ID: 43, Version: 0)
/// 
/// MAC Address Setup 配置 MAC 地址。
#[derive(Debug, Clone)]
...
impl CosemObject for MacAddressSetup {
    ...
    fn get_attribute(&self, attr_id: u8) -> Result<DlmsType, CosemError> {
        match attr_id {
            ...
            2 => Ok(self.mac_address.clone().into()),
...
mod tests {
    ...
    #[test]
    fn test_mac__address__setup_class_id() {
        assert_eq!(MacAddressSetup::CLASS_ID, 43);
    }

    #[test]
    fn test_mac__address__setup_version() {
        assert_eq!(MacAddressSetup::VERSION, 0);
    }

    #[test]
    fn test_mac__address__setup_new() {
        let obis = ObisCode::from_str("0.0.43.0.0.255").unwrap();
        let obj = MacAddressSetup::new(obis.clone());
        assert_eq!(obj.logical_name, obis);
    }

    #[test]
    fn test_mac__address__setup_get_logical_name() {
        let obis = ObisCode::from_str("0.0.43.0.0.255").unwrap();
        let obj = MacAddressSetup::new(obis.clone());
    ...
    #[test]
    fn test_mac__address__setup_get_mac_address() {
        let obj = MacAddressSetup::new(ObisCode::from_str("0.0.43.0.0.255").unwrap());
